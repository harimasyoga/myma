			...
			<form role="form" method="post" id="myForm">
				<div class="card-body">		
					<table id="datatable" class="table table-bordered table-striped" width="100%">
						...
						<input type="hidden" name="id_cek" id="id_cek" value="<?= $id ?>">
						<input type="hidden" name="status" id="status" value="Add">
						<input type="hidden" name="jenis" id="jenis" value="edit_checklist">
						<tbody>
							...
							<?php
								...
								<tr class="show1" id="row">
									...
									<td align="center" >
										<?php $query_cek = $this->db->query("SELECT*FROM  m_check_perkara_detail where id_cek='$id' and kode_modul='$row->id_check' ")->num_rows(); ?>
											
										<input id="<?= "status".$row->id_check;?>"  name="<?= "status".$row->id_check;?>" type="checkbox" class="form-control" onchange="cek(<?= $row->id_check ?>,this.value)"
					...
					<br>
					<a href="<?= base_url('Master/Checklist')?>" class="btn btn-sm btn-danger"><i class="fa fa-chevron-left"></i> <b>Kembali</b></a>
		...
		$.ajax({
			...
			success: function(data) {
				if (data) {
					...
					window.location.href = '<?php echo base_url('Master/Checklist')?>'
				} else {
					swal.close();
					swal({
						title               : "Cek Kembali",
