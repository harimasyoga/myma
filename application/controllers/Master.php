...
class Master extends CI_Controller
{
	...
	function Pegawai()
	{
		...
		$this->load->view('header', $data);
		$this->load->view('Master/v_pegawai', $data);
		$this->load->view('footer');
	}
	
	function List_terdakwa()
	{
		$data = array(
			'judul' => "Master List Terdakwa"
		);
		$this->load->view('header', $data);
		$this->load->view('Master/v_list_terdakwa', $data);
		$this->load->view('footer');
	}
	...
	{
		$data = array(
			'judul' => "Master Checklist"
		);

		$this->load->view('header', $data);
		$this->load->view('Master/v_checklist', $data);
		$this->load->view('footer');
	}
	...
	function cetak_check()
	{
		...
		$param      = $judul;
        $query_header = $this->db->query("SELECT*from m_check_perkara where id_check not in (1,2,3) order by id_check");
		...
		if ($query_header->num_rows() > 0) 
		{
			...
			$html .= '<table width="100%" border="0" cellspacing="0" style="font-size:14px;font-family: ;">
				...
				$html .= '<table width="100%" border="1" cellspacing="1" cellpadding="3" style="border-collapse:collapse;font-size:12px;font-family: ;">
						...
						';
						foreach ($query_header->result() as $header_l) 
						{
						$html .= '
							<th align="center">'.$header_l->nama.'</th>';
						}
						$html .= '
						</tr>';
			...
			$query_detail = $this->db->query("SELECT id_cek,no_perkara,pp, 
				IFNULL((select id_check_d from m_check_perkara_detail b where b.id_cek=a.id_cek and kode_modul='$header_l->id_check'),'0') id_check_d
				from m_checklist a where pp in ('hendra') order by id_cek");
			...
			foreach ($query_detail->result() as $r) 
			{
				...
				// {
					...
					$html.='<tr>
						<td align="center">' . $r->no_perkara . '</td>';

					foreach ($query_header->result() as $header_l2) 
					{
						$query_detail2 = $this->db->query("SELECT*from m_check_perkara_detail where id_cek='$r->id_cek' and kode_modul='$header_l2->id_check'");
						if ($query_detail2->num_rows() > 0) 
						{							
							$html.='
									<td align="center" style="background-color:#36cfd1">
		...
		switch ($cekpdf) {
			...
			case 1;
				$this->M_fungsi->_mpdf_hari($position, 'A4', $judul, $html, 'REKAP.pdf', 5, 5, 5, 10);
	...
	function kelengkapan_berkas()
	{
		
		$id_cek   = $_GET['id_cek'];
		$nama     = $this->db->query("SELECT*FROM m_checklist a WHERE id_cek='$id_cek' LIMIT 1 ")->row();
		
		$query = $this->db->query("SELECT * FROM m_check_perkara a order by id_check")->result();

		$data = array(
			'judul'  => "Edit Checklist",
			'judul2' => "$nama->no_perkara",
			'id'     => $id_cek,
			'query'  => $query
		);
		...
		$this->load->view('header', $data);
		$this->load->view('Master/v_kelengkapan_perkara', $data);
		$this->load->view('footer');
	}
	...
	function load_data()
	{
		...
		if ($jenis == "hub") {
			...
			$query = $this->m_master->query("SELECT * FROM m_pegawai ORDER BY id_peg")->result();
			$i = 1;
			foreach ($query as $r) {
				$row = array();
				$row[] = '<div class="text-center"><a href="javascript:void(0)" onclick="tampil_edit('."'".$r->id_peg."'".','."'detail'".')">'.$i.'<a></div>';
				$row[] = $r->nm_peg;
				$row[] = $r->nip;
				$row[] = $r->jabatan;

				$btnEdit = '<button type="button" class="btn btn-warning btn-sm" onclick="tampil_edit('."'".$r->id_peg."'".','."'edit'".')"><i class="fas fa-pen"></i></button>';

				$btnHapus = '<button type="button" class="btn btn-danger btn-sm" onclick="deleteData('."'".$r->id_peg."'".')"><i class="fas fa-times"></i></button>';
			...
			if($thnn=='all')
			{
				...
				$thn=" WHERE th_perkara='$thnn' ";

			}

			$query = $this->m_master->query("SELECT * FROM m_terdakwa_lama $thn ORDER BY id_terdakwa")->result();
			$i = 1;
			foreach ($query as $r) {
				$row = array();
				$row[] = '<div class="text-center"><a href="javascript:void(0)" onclick="tampil_edit('."'".$r->id_terdakwa."'".','."'detail'".')">'.$i.'<a></div>';
				$row[] = $r->no_perkara;
				$row[] = $r->nm_terdakwa;
				$row[] = $r->ttl;
				$row[] = $r->th_perkara;

				$btnEdit = '<button type="button" class="btn btn-warning btn-sm" onclick="tampil_edit('."'".$r->id_terdakwa."'".','."'edit'".')"><i class="fas fa-pen"></i></button>';

				$btnHapus = '<button type="button" class="btn btn-danger btn-sm" onclick="deleteData('."'".$r->id_terdakwa."'".')"><i class="fas fa-times"></i></button>';
		...
		} else if ($jenis == "checklist") {
			
			$query = $this->m_master->query("SELECT * FROM m_checklist ORDER BY id_cek")->result();
			$i = 1;
			foreach ($query as $r) {
				$row = array();

				$row[] = '<div class="text-center">'.$r->id_cek.'</div>';
				$row[] = $r->no_perkara;
				$row[] = $r->pp;
				// if($r->ket=='Y')
				// {
				// 	$row[] = '<a href="javascript:void(0)" class="btn btn-success btn-sm"><i class="fas fa-check"></i></a>';
				// }else{
				// 	$row[] = '<a href="javascript:void(0)" class="btn btn-danger btn-sm"><i class="fas fa-lock"></i></a>';
				// }
				...
				$row[] = '<div class="text-center"><a style="text-align: center;" class="btn btn-sm '.$btn2.' " title="VERIFIKASI DATA" ><b>'.$i2.' </b> </a><span style="font-size:1px;color:transparent"></span><div>';

				

				$row[] = '<div class="text-center">
				<a href="javascript:void(0)" onclick="tampil_edit(' . "'" . $r->id_cek . "'" . ',' . "'edit'" . ')" class="btn btn-warning btn-sm">
				<i class="fas fa-edit"></i>
				</a>

				<a class="btn btn-sm btn-primary" href="'. base_url("Master/kelengkapan_berkas?id_cek=" . $r->id_cek . "") .'" title="EDIT MENU" ><b>
				<i class="fas fa-search"></i></b></a>
